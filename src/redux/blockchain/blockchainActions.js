...
export const connect = (walletName = '') => {
  ...
  return async (dispatch) => {
    ...
    // NFT ABI
    const abiResponse = await fetch("/config/nft_abi.json", {
      headers: {
    ...
    // Staking ABI
    const stakeAbiResponse = await fetch("/config/stake_abi.json", {
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
      },
    ...
    if (metamaskIsInstalled || coinbaseIsInstalled) {
      ...
      try {
        ...
        const StakeSmartContractObj = new Web3EthContract(
          stakeAbi,
        ...
        dispatch(
          connectSuccess({
            ...
            stakeSmartContract: StakeSmartContractObj,
            web3: web3,
